...
exports.getFriendRequests = exports.getFriends = exports.removeFriend = exports.rejectFriendRequest = exports.acceptFriendRequest = exports.sendFriendRequest = exports.searchUsers = exports.updateProfile = exports.getProfile = void 0;
...
const getProfile = async (req, res) => {
    try {
        ...
        if (userId && userId !== req.user.id) {
            ...
            const myFriends = await database_1.prisma.friendship.findMany({
...
const updateProfile = async (req, res) => {
    try {
        ...
        if (req.body.password) {
            ...
            const validPassword = await bcryptjs_1.default.compare(req.body.currentPassword, user.password);
            if (!validPassword) {
        ...
        if (req.files) {
            if (req.files.avatar && req.files.avatar[0]) {
                ...
                const cloudinaryResult = await (0, cloudinary_1.uploadToCloudinary)(avatarFile.path, 'avatars', {
                    width: 400,
                    height: 400,
                    crop: 'fill',
                    quality: 'auto',
                ...
                await promises_1.default.unlink(avatarFile.path).catch(console.error);
            }
            if (req.files.coverPhoto && req.files.coverPhoto[0]) {
                const coverFile = req.files.coverPhoto[0];
                const cloudinaryResult = await (0, cloudinary_1.uploadToCloudinary)(coverFile.path, 'covers', {
                    width: 1500,
                    height: 500,
                    crop: 'fill',
                    quality: 'auto',
                ...
                // Delete local file after upload
                await promises_1.default.unlink(coverFile.path).catch(console.error);
            }
        }
...
const sendFriendRequest = async (req, res) => {
    try {
        ...
        const friendship = await database_1.prisma.friendship.create({
            data: {
                userId: req.user.id,
                friendId: userId,
                status: 'pending'
...
const acceptFriendRequest = async (req, res) => {
    try {
        ...
        const friendship = await database_1.prisma.friendship.findUnique({
            where: { id: requestId },
            include: {
                user: true,
                friend: true
        ...
        // Accept friendship
        await database_1.prisma.friendship.update({
            where: { id: requestId },
            data: { status: 'accepted' }
        });
        // Create reverse friendship for bidirectional relationship
        const reverseFriendship = await database_1.prisma.friendship.findFirst({
            where: {
                userId: req.user.id,
        ...
        if (!reverseFriendship) {
            await database_1.prisma.friendship.create({
                data: {
                    userId: req.user.id,
                    friendId: friendship.userId,
                    status: 'accepted'
...
const rejectFriendRequest = async (req, res) => {
    try {
        ...
        await database_1.prisma.friendship.delete({
            where: { id: requestId }
        });
        res.json({
            success: true,
...
const getFriends = async (req, res) => {
    try {
        ...
        const total = await database_1.prisma.friendship.count({
            where: {
                OR: [
                    { userId: targetUserId, status: 'accepted' },
                    { friendId: targetUserId, status: 'accepted' }
        ...
        const friends = friendships.map(f => f.userId === targetUserId ? f.friend : f.user);
        res.json({
            success: true,
            data: friends,
            pagination: {
...
const getFriendRequests = async (req, res) => {
    try {
        ...
        const requests = await database_1.prisma.friendship.findMany({
            where,
            include: type === 'sent'
                ? {
                    friend: {
        ...
        const total = await database_1.prisma.friendship.count({ where });
        res.json({
            success: true,
            data: {
                requests,
